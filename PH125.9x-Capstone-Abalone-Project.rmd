...
In order to determine the best model for predicting rings, and hence the highest accuracy, both regression and classification analyses were done.  Models used for regression are:  lm, glm, knn, svmLinear, rpart, gamLoess, treebag, gbm, and rf.  The classification models are:  knn, lda, qda, naive_bayes, svmLinear, svmRadial, gamLoess, multinom, and rf.  Classification was done by grouping rings into age groups:  juvenile, adult, and senior.  The best models (least RMSE / highest accuracy) were chosen from each set of analysis.  The results and conclusions from the analysis are reported.
...
tmptxt <- paste0("Regression and classification analyses were done for multiple models.  The best result is obtained with classification using the \"",
        class_fits[[best_class]]$method,
...
Additional work can be done on tuning the models, exploring other methods, and/or including additional data, such as weather, water conditions, and food availability.
